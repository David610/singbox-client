...
void main() {
  ...
  test('failed migration leaves the old configuration byte-for-byte usable', () async {
    final plaintext = jsonEncode(profile('one', 'uuid-one', 'password-one'));
  ...
  test('missing and corrupted entries fail closed', () async {
    ...
    await expectLater(store.readAndMigrate(file), throwsA(isA<CredentialStoreException>()));
  ...
  test('multiple profiles, rotation, replacement, and deletion reconcile secrets', () async {
    final two = {
      'items': [
  ...
  test('settings credentials use an isolated namespace', () async {
    ...
    final settingsStore = ProfileCredentialStore(backend, namespace: 'settings');
    await profileStore.write(file, profile('one', 'uuid-one', 'password-one'));
    final settingsFile = File('${directory.path}/setting.json');
    final settings = {
      'webdav': {'url': 'https://dav.invalid', 'user': 'alice', 'password': 'dav-secret'},
