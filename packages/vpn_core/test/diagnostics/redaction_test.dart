...
void main() {
  group('key-value redaction (JSON shape)', () {
    test('redacts uuid', () {
      const input = '{"type":"vless","uuid":"9c7e12d1-64c3-46f2-9e21-d707f05c88d9"}';
    ...
    test('redacts REALITY public_key and short_id', () {
      ...
      final out = redactText(input);
      expect(out.contains('anIGqfcDa8ypMGtP6lcoc3Fu54p3gOWMl9LKvIjRx3w'), isFalse);
    ...
    test('redacts private_key', () {
      const input = '{"private_key":"6BCFEAfj1E78_XMA9Ue0gkg0td145QdwizDSJ_fjvVA"}';
      expect(redactText(input).contains('6BCFEAfj1E78'), isFalse);
    });
  });

  group('key-value redaction (query-string shape, vless:// / hysteria2:// URIs)', () {
  ...
  group('key-value redaction (log-line shape)', () {
    test('redacts "key: value" style engine log lines', () {
      const line = 'starting vless outbound uuid: 9c7e12d1-64c3-46f2-9e21-d707f05c88d9';
      expect(redactText(line).contains('9c7e12d1'), isFalse);
    });

    test('redacts subscription token / auth token mentions', () {
      const line = 'fetching subscription, token: abcDEF0123456789abcDEF0123456789';
  ...
  group('shape-aware defense in depth', () {
    test('redacts a standalone UUID with no surrounding key context', () {
      const input = 'correlating profile 9c7e12d1-64c3-46f2-9e21-d707f05c88d9 with server X';
      expect(redactText(input).contains('9c7e12d1'), isFalse);
    });

    test('redacts a long base64url-shaped token with no known key name', () {
      const input = 'unexpected value: qvVV9d8L22TV7uplmwiiUj57UPeRpixH5yMavAy7tgoXYZ';
      final out = redactText(input);
      expect(out.contains('qvVV9d8L22TV7uplmwiiUj57UPeRpixH5yMavAy7tgoXYZ'), isFalse);
    });

    test('redacts an unfamiliar credential-shaped field name via the shape heuristic', () {
      ...
      // catch it.
      const input = '{"futureSecretField":"aVeryLongRandomLookingCredentialValue123456"}';
      final out = redactText(input);
      expect(out.contains('aVeryLongRandomLookingCredentialValue123456'), isFalse);
    });
  });
