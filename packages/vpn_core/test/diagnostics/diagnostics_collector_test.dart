...
void main() {
  ...
  test(
    'captureStatic reflects current VPN state and never leaks the UUID',
    () async {
      fake.emit(
        const VpnCoreStatus(
  ...
  test(
    'reconnect count increments only on a genuine re-connection, not the first connect',
    () async {
      collector = DiagnosticsCollector();
      fake.emit(const VpnCoreStatus(state: VpnCoreState.connecting));
      fake.emit(
        const VpnCoreStatus(state: VpnCoreState.connected, activeTag: 't'),
      );
  ...
  test(
    'tunnel uptime is null when disconnected and non-null once connected',
    () async {
      collector = DiagnosticsCollector();
      var snap = await collector.captureStatic(app);
  ...
  test(
    'lastEngineError picks the most recent error-looking log line, sanitized',
    () async {
      fake.logs = [
        '[info] starting',
  ...
  test(
    'lastEngineError is null when no log line looks like an error',
    () async {
  ...
  test(
    'captureWithProbes only queries public IP when explicitly requested',
    () async {
      ...
      final withIp = await collector.captureWithProbes(
        app,
  ...
  test(
    'capturePublicIpBaseline sets publicIpBefore on later snapshots',
    () async {
