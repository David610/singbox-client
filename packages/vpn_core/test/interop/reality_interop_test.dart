...
void main() {
  ...
  test(
    'VLESS+REALITY: generated client outbound completes a real handshake '
    'and carries an HTTP GET',
    () async {
        ...
        'inbounds': [
          {
            ...
            'users': [
              {'name': 'interop-test-user', 'uuid': _testUuid, 'flow': 'xtls-rprx-vision'},
            ],
            'tls': {
              'enabled': true,
              ...
              'reality': {
                'enabled': true,
                'handshake': {'server': LocalDecoy.hostname, 'server_port': decoy.port},
      ...
      await File(serverConfigPath).writeAsString(const JsonEncoder.withIndent('  ').convert(serverConfig));
      ...
      await File(clientConfigPath).writeAsString(const JsonEncoder.withIndent('  ').convert(clientConfig));
      ...
      expect(response, isNotNull, reason: 'real VLESS+REALITY tunnel should have carried the request');
  ...
  test(
    'VLESS+REALITY: a wrong (but well-formed) REALITY public key is '
    ...
    () async {
      ...
      expect(response, isNull, reason: 'a wrong REALITY public key must not tunnel traffic');
    },
