...
void main() {
  ...
  group(
    'vless:// (REALITY, flow NOT supplied) -- vless_reality_uri_vision_off.txt',
    () {
  ...
  group('hysteria2:// (with Salamander obfs) -- hysteria2_uri.txt', () {
    ...
    test('every field survives into the generated core outbound config', () {
      ...
      expect(
        tls.containsKey('insecure'),
        isFalse,
        reason:
            'insecure=0 in the URI must not force an "insecure" key '
  ...
  group('sing-box JSON subscription -- subscription_singbox.json', () {
    ...
    test(
      'contains both a vless and a hysteria2 outbound, plus routing groups',
      () {
        final types = outbounds.map((o) => o['type']).toSet();
        expect(
          types,
          containsAll(['vless', 'hysteria2', 'urltest', 'selector', 'direct']),
        );
      },
    );

    test('the vless/REALITY outbound has no field silently dropped', () {
      final vless = SingBoxConfigBuilder.findOutboundByType(
        outbounds,
        'vless',
      )!;
    ...
    test('the hysteria2 outbound has no field silently dropped', () {
      final hy2 = SingBoxConfigBuilder.findOutboundByType(
        outbounds,
        'hysteria2',
      )!;
  ...
  group('subscription URL import shape', () {
    test('a fetched subscription body (this fixture stands in for the HTTP '
      ...
      final outbounds = SingBoxConfigBuilder.extractOutboundsFromSubscription(
        body,
      );
