...
/// Implements both Go interfaces the pinned core calls back into from
/// inside the packet-tunnel process:
///   - `PlatformInterface` (experimental/libbox/platform.go) -- TUN setup,
///     DNS/route application, interface monitoring, WiFi/cert/notification
///     hooks.
///   - `CommandServerHandler` (experimental/libbox/command_server.go) --
///     stop/reload requests and system-proxy queries the core's own
///     command protocol issues back to the host.
///
/// Every method below, its exact Swift signature, and the specific
/// `NEPacketTunnelNetworkSettings`/`NEIPv4Settings`/`NEIPv6Settings`/
/// `NEProxySettings` construction in `openTun0` is transcribed from
/// SagerNet/sing-box-for-apple's `Library/Network/ExtensionPlatformInterface.swift`
/// at its `main` branch (`MARKETING_VERSION = 1.13.19`, verified to match
/// this project's pin -- both the Swift source and the Go interface
/// declarations at `github.com/sagernet/sing-box@v1.13.19` were read in
/// full, not guessed). Trimmed relative to that reference: no macOS/tvOS
/// branches (this app is iOS-only), no `RootHelperClient`/XPC system-
/// extension path, no per-profile `OverridePreferences` struct -- this
/// app's `VpnCorePlugin.swift` exposes no UI for those toggles, so the
/// constants below reproduce upstream's own *default* values for an
/// unconfigured profile (`OverridePreferences()`'s defaults) rather than
...
final class ExtensionPlatformInterface: NSObject, LibboxPlatformInterfaceProtocol, LibboxCommandServerHandlerProtocol {
    ...
    // Matches `ExtensionProvider.OverridePreferences()`'s defaults in the
    // upstream reference -- this app exposes no UI to override them.
    private let autoRouteUseSubRangesByDefault = false
    private let excludeAPNsRoute = false
    private let excludeDefaultRoute = false
    private let systemProxyEnabled = true
    ...
    // MARK: - PlatformInterface: TUN setup
    ...
    private func openTun0(_ options: LibboxTunOptionsProtocol?, _ ret0_: UnsafeMutablePointer<Int32>?) async throws {
        ...
        if options.getAutoRoute() {
            ...
            if excludeDefaultRoute, !ipv4Routes.isEmpty {
                if !ipv4ExcludeRoutes.contains(where: { $0.destinationAddress == "0.0.0.0" && $0.destinationSubnetMask == "255.255.255.254" }) {
                    ipv4ExcludeRoutes.append(NEIPv4Route(destinationAddress: "0.0.0.0", subnetMask: "255.255.255.254"))
                }
            }
            if excludeAPNsRoute, !ipv4Routes.isEmpty {
                if !ipv4ExcludeRoutes.contains(where: { $0.destinationAddress == "17.0.0.0" && $0.destinationSubnetMask == "255.0.0.0" }) {
            ...
            if excludeDefaultRoute, !ipv6Routes.isEmpty {
                if !ipv6ExcludeRoutes.contains(where: { $0.destinationAddress == "::" && $0.destinationNetworkPrefixLength == 127 }) {
            ...
            let hasDefaultRoute = ipv4Routes.contains(where: { $0.destinationAddress == "0.0.0.0" && $0.destinationSubnetMask == "0.0.0.0" })
            if !hasDefaultRoute {
        ...
        if options.isHTTPProxyEnabled() {
            ...
            if excludeAPNsRoute, !bypassDomains.contains("push.apple.com") {
        ...
        // `packetFlow.value(forKeyPath: "socket.fileDescriptor")` is the
        // real mechanism sing-box-for-apple uses to obtain the raw TUN fd
        // from `NEPacketTunnelProvider.packetFlow` -- Apple exposes no
        // public accessor for it, unlike Android's VpnService.Builder
        // which returns an explicit ParcelFileDescriptor.
        // `LibboxGetTunnelFileDescriptor()` is the documented fallback for
        // when that private KVC path is unavailable.
    ...
    // MARK: - PlatformInterface: everything else
    ...
    func findConnectionOwner(_ ipProtocol: Int32, sourceAddress: String?, sourcePort: Int32, destinationAddress: String?, destinationPort: Int32) throws -> LibboxConnectionOwner {
        // No root-helper/XPC process-owner lookup exists on iOS (that is a
        // macOS-only capability in the upstream reference too); sing-box
        // treats this as "unsupported on this platform", not a fatal
        // error, when process-matching rules aren't used.
    ...
    // No app UI exposes an "include all networks" (iOS's full VPN-mode)
    // toggle; the tunnel's own configured routes already decide what's
    // proxied, matching upstream's default for an unconfigured profile.
    ...
    func setSystemProxyEnabled(_ isEnabled: Bool) throws {
        ...
        if proxySettings.httpEnabled == isEnabled { return }
    ...
    // No local DNS transport hook or platform trust-anchor enumeration is
    // wired up -- matches the verified upstream reference, which also
    // returns nil for both on Apple platforms (sing-box falls back to its
    // own resolver / Go's standard TLS trust store in that case; see
    // experimental/libbox/{dns,platform}.go).
...
private final class NetworkInterfaceArrayIterator: NSObject, LibboxNetworkInterfaceIteratorProtocol {
